...
class Compiler():
    ...
    # a temporary test function that will only be used for debugging
    def test(self, ast):
        ast_head = ast[0]
    ...
    # get_body :: [AST_Node] -> ([AST_Node],[AST_Node])
    ...
    def c_function_def(self, asts:List[AST_Node], assembly:List[str], labels: dict, scope: dict) -> Tuple[List[AST_Node], List[str]]:
        ...
        if (isinstance(asts[0],AST_Function)):
            ...
            assembly = assembly + ["    .global " + asts[0].procedure_name,asts[0].procedure_name + ":"] + rslt # prepend the procedure name
