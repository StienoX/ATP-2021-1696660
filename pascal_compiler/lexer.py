...
# Functions:
#
# lex_something: implements a lexer function that expects a lambda function to make its return type
...
# it returns a modified version of data when the first check passses.
#
# lex_something_between: implements a lexer function that expects a lambda function to make its return type
# it expects the following parameters
...
#
#
# lex_*: these functions lexes each token using the underlying *_something functions. The type of this function is (String, [Token]) -> (String, [Token])
# lex_all: runs all lex_* functions until the no input is left. If an invalid program is provided this function will crash.
# run: wrapper around the lex_all function
class Lexer():
    def __init__(self):
        self.types =            ['integer', 'real', 'char', 'boolean', 'string']
        self.keywords =         ['var', 'repeat', 'while', 'do', 'until', 'WriteLn', 'begin', 'end', 'function', ',', ';', 'program', 'if', 'then', 'else', 'ReadLn']
        ...
        # composition of all the lex functions (used in lex_all)
