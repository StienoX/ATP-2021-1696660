...
# Functions:
# The functions are diveded by start parsers, follow-up parsers and closing parsers
# Start parsers are parsers that can be called from other parsers. These parsers are calling follow-up parsers.
# Follow-up parsers are parsers called from another parsering the same block. These follow-up parsers themselfes can call 
...
class Parser():
    ...
    # it inserts these tokens into the expression tree based on precendence
    ...
    def p_fu_expression(self, data: Tuple[List[Token],AST_Node], head_node: AST_Expression, open: bool) -> Tuple[List[Token],AST_Node]: 
        ...
        # case 2: We have found a node equal or larger than our own nodes precendence - insert the new node at the current position and add the rest of the tree as our left node
